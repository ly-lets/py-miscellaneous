...
def calculate_total_word_count(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            ...
            #     counted_files.append((file_name, count))
                ...
                not_counted_fles.append((file_name, 0))
    counted_files.extend(not_counted_fles)
    file_collection = [
    ...
    return word_count, file_collection


def main():
    print(f">>>>>>>>>>>>>>>>>>>>>> Counter is ramping up <<<<<<<<<<<<<<<<<<<<")
    target_directory = ""
    if len(sys.argv) < 2:
    ...
    total_word_count, counted_files = calculate_total_word_count(target_directory)
